...
router.post('/c/post/d', currentUser, requireAuth, async (req, res) => {
 ...
 try {
  ...
  if(post.down_votes.includes(id)){
    ...
    post.vote_count.count = post.vote_count.count - 1
    post.vote_count.downvote_count = post.vote_count.downvote_count - 1
    await post.save()
  }
  // If user upvoted before but the selects to downvote, remove upvote and place in downvote
    ...
    post.vote_count.upvote_count = post.vote_count.upvote_count - 1
